...
import { HourWorkedProposalApi } from "./api/HourWorkedProposalApi";
...
export default function HoraProposta() {
  const columns = [
  { field: 'data', headerName: 'Data', width: 200 },    
  { field: 'id_employee', headerName: 'Funcionario', width: 200 },    
  { field: 'id_proposal', headerName: 'Proposta', width: 200 },    
...
const AllProposals = async () => {
  const result = await ApiGlobal.getProposals();
  setProposals(result);
}; 
const allHoursWorkedProposal = async () => {
  const results = await HourWorkedProposalApi.get();
  setItems(results);
};
  
useEffect(() => { 
  allHoursWorkedProposal();
}, []);
...
const create = async () => {
  ...
  if(dado.id > 0){
    await HourWorkedProposalApi.update(dado)      
  }
  else{
    await HourWorkedProposalApi.create(dado)
  }
  allHoursWorkedProposal()
  fecharModal()
};
...
const remover = (id) => {
  HourWorkedProposalApi.delete(id)
 .then(dado=>{
   allHoursWorkedProposal()
 })
}
...
async function edit(id){
  const selected = await HourWorkedProposalApi.getId(id)
  
  abrirModal()
  setId(selected.id)
  setData(selected.data)
  setId_employee(selected.id_employee)
  setId_proposal(selected.id_proposal)
...
const[id_proposal, setId_proposal] = useState('')
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>                  
                <Select
                  ...
                  value={id_proposal}
                  onChange={(event)=>{
                    setId_proposal(event.target.value)
                  }}
                  >     
